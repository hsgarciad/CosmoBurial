function [Tburial, Tburial_unc, Ero, Ero_unc] = SimpleBurial_muons2exp(be10, be10_unc, al26, al26_unc, lat, lon, elev, mass_depth)
% SimpleBurial_muons
...
% Muon production at surface (Balco 2017, two-exponential fit)
% P and L are kept separate so each term attenuates independently with depth.
% This avoids the lumped-Pz ratio instability at large mass depths.
gmr  = -0.03417;
dtdz = 0.0065;
...
%% ===== Monte Carlo =====
% Solves for [E (g/cm2/yr), T (yr)].
%
% Full equation per nuclide (instantaneous burial at fixed depth z):
%
%   N = inherited_spal * decay
%     + inherited_mu1  * decay                (muon term 1, attenuated to depth)
%     + inherited_mu2  * decay                (muon term 2)
%     + post_spal                             (spallation at depth — negligible for z>>Lspal)
%     + post_mu1                              (muon term 1 at depth, accumulated over T)
%     + post_mu2                              (muon term 2 at depth, accumulated over T)
%
% Inherited:  Ps/(lambda + E/L) * exp(-T/Tmean)
% Post-burial muon k at depth z:  Pmk * exp(-z/Lmk) * Tmean * (1 - exp(-T/Tmean))
...
for i = 1:n
    ...
    % Step 2: full solve with two-exponential muon terms
    F = @(x) [ ...
        (Ps_Al  / (lambda_Al + x(1)/Lspal )) * exp(-x(2)/TAl) + ...
        (Pm1_Al / (lambda_Al + x(1)/Lm1_Al)) * exp(-x(2)/TAl) + ...
        (Pm2_Al / (lambda_Al + x(1)/Lm2_Al)) * exp(-x(2)/TAl) + ...
        Pm1_Al * exp(-z/Lm1_Al) * TAl * (1 - exp(-x(2)/TAl)) + ...
        Pm2_Al * exp(-z/Lm2_Al) * TAl * (1 - exp(-x(2)/TAl)) - Al_rand; ...
        ...
        (Ps_Be  / (lambda_Be + x(1)/Lspal )) * exp(-x(2)/TBe) + ...
        (Pm1_Be / (lambda_Be + x(1)/Lm1_Be)) * exp(-x(2)/TBe) + ...
        (Pm2_Be / (lambda_Be + x(1)/Lm2_Be)) * exp(-x(2)/TBe) + ...
        Pm1_Be * exp(-z/Lm1_Be) * TBe * (1 - exp(-x(2)/TBe)) + ...
        Pm2_Be * exp(-z/Lm2_Be) * TBe * (1 - exp(-x(2)/TBe)) - Be_rand  ...
    ];
    [sol, ~, exitflag] = fsolve(F, sol0, opts);

    % Sanity checks:
    % 1. Muon-corrected age must be >= no-muon age
    % 2. Modeled Al/Be ratio must lie between production ratio and measured ratio
    ratio_prod = Ps_Al / Ps_Be;
    ratio_meas = Al_rand / Be_rand;
    Be_mod = (Ps_Be  / (lambda_Be + sol(1)/Lspal )) * exp(-sol(2)/TBe) + ...
             (Pm1_Be / (lambda_Be + sol(1)/Lm1_Be)) * exp(-sol(2)/TBe) + ...
             (Pm2_Be / (lambda_Be + sol(1)/Lm2_Be)) * exp(-sol(2)/TBe) + ...
             Pm1_Be * exp(-z/Lm1_Be) * TBe * (1 - exp(-sol(2)/TBe)) + ...
             Pm2_Be * exp(-z/Lm2_Be) * TBe * (1 - exp(-sol(2)/TBe));
    Al_mod = (Ps_Al  / (lambda_Al + sol(1)/Lspal )) * exp(-sol(2)/TAl) + ...
             (Pm1_Al / (lambda_Al + sol(1)/Lm1_Al)) * exp(-sol(2)/TAl) + ...
             (Pm2_Al / (lambda_Al + sol(1)/Lm2_Al)) * exp(-sol(2)/TAl) + ...
             Pm1_Al * exp(-z/Lm1_Al) * TAl * (1 - exp(-sol(2)/TAl)) + ...
             Pm2_Al * exp(-z/Lm2_Al) * TAl * (1 - exp(-sol(2)/TAl));
    ratio_mod = Al_mod / Be_mod;
    ratio_ok  = ratio_mod >= min(ratio_meas, ratio_prod) && ...
                ratio_mod <= max(ratio_meas, ratio_prod);

    if exitflag > 0 && sol(2) >= sol0(2) && isreal(sol(2)) && sol(1) >= 0 && ratio_ok
        j = j + 1;
        Tburial_dis(j) = sol(2);
        E_dis(j)       = sol(1);
    end
